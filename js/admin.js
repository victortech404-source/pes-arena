// UON HUB/js/admin.js - Updated with Role-Based Governance and PES ARENA Branding
class AdminManager {
    constructor() {
    ...
    switchTab(tabName) {
        ...
        if (tabName === 'matches') this.loadMatchResults();
        if (tabName === 'applications') this.loadPendingRequests();
        if (tabName === 'news') this.loadNewsPosts();
    }
    ...
    // --- TAB 1: APPLICATIONS (UPDATED with organizer filter) ---
    ...
    // --- TAB 2: MANAGE TOURNAMENTS (UPDATED with organizer filter) ---
    ...
    async loadMatchResults() {
        ...
        try {
            ...
            if (snapshot.empty) {
                container.innerHTML = '<div class="empty-state"><span class="empty-icon">✓</span><h3>All Caught Up</h3><p>No pending match results to review.</p></div>';
                return;
            }
            ...
            snapshot.forEach(doc => {
                ...
                html += `
                    <div class="match-card">
                        ...
                        <div class="action-row">
                            <button onclick="window.adminManager.approveMatch('${doc.id}', '${match.userId}')" class="btn-approve">
                                ✅ Approve Result
                            </button>
                            <button onclick="window.adminManager.rejectMatch('${doc.id}')" class="btn-reject">
                                🗑️ Reject
                            </button>
                        </div>
                    </div>`;
    ...
    // --- UPDATED MATCH APPROVAL FUNCTION ---
    ...
    // --- TAB 4: CREATE TOURNAMENT WITH IMAGE UPLOADS (UPDATED with organizerId) ---
    setupFormListeners() {
    ...
    async handleCreateTournament() {
        ...
        try {
            ...
            const data = {
                ...
                // Step 11: Store organizer information
    ...
    // --- FIXED SYNC OLD STATS FUNCTION (UPDATED with new notification text) ---
    async syncOldStats() {
        const result = await Swal.fire({
        ...
        try {
            ...
            // 5. Update users in batches to avoid Firebase limits
    ...
    // Helper function to update individual player goal stats
    async updatePlayerGoalStats(id, scored, conceded) {
        if (!id) return;
        const ref = this.db.collection('users').doc(id);
        const doc = await ref.get();
        if (doc.exists) {
            const d = doc.data();
            const newScored = (d.goalsScored || 0) + scored;
            const newConceded = (d.goalsConceded || 0) + conceded;
            await ref.update({
                goalsScored: newScored,
                goalsConceded: newConceded,
                goalDifference: newScored - newConceded
            });
        }
    }
}
